...
--   - La PK: siempre BIGINT GENERATED ALWAYS AS IDENTITY
--
...
--     CONSTRAINT ??? CHECK (publication_year > 0)
-- );
...
--     full_name           ???  NOT NULL,
--     email               ???  NOT NULL,
--     registration_date   ???  NOT NULL DEFAULT ???,
--
...
--
--     CONSTRAINT ???
--         FOREIGN KEY (book_id) REFERENCES bootcamp.books(id)
--         ON DELETE ???,
--
--     CONSTRAINT ???
--         FOREIGN KEY (member_id) REFERENCES bootcamp.members(id)
--         ON DELETE ???
-- );
...
-- INSERT INTO bootcamp.books (title, author, isbn, publication_year) VALUES
--     ('???', '???', '???', ???),
--     ('???', '???', '???', ???),
--     ('???', '???', '???', ???);
...
-- INSERT INTO bootcamp.members (full_name, email) VALUES
--     ('???', '???'),
--     ('???', '???');
...
SELECT * FROM bootcamp.books ORDER BY id;

-- Ver todos los socios
SELECT * FROM bootcamp.members ORDER BY id;

-- Ver todos los préstamos con información del libro y el socio
SELECT
    l.id            AS prestamo_id,
    b.title         AS libro,
    m.full_name     AS socio,
...
JOIN bootcamp.books    AS b  ON b.id = l.book_id
JOIN bootcamp.members  AS m  ON m.id = l.member_id
