...
| `FOREIGN KEY` | Ver *clave foránea*. | `CONSTRAINT fk_loans_book_id FOREIGN KEY (book_id) REFERENCES books(id)` |
| `Fila` | Término coloquial para *tupla*. Un registro individual en una tabla. | Cada libro en la tabla `books` ocupa una fila. |
| `GENERATED ALWAYS AS IDENTITY` | Sintaxis moderna de PostgreSQL para columnas auto-incrementales; el valor lo genera siempre el motor (no puede insertarse manualmente). | `id BIGINT GENERATED ALWAYS AS IDENTITY` |
...
| `Integridad referencial` | Propiedad que garantiza que toda clave foránea apunta a una fila existente en la tabla referenciada. | No puede existir un `loan` con `book_id = 99` si no hay ningún libro con `id = 99`. |
